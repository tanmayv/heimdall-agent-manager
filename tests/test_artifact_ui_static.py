...
checks = [
    ...
    ('chain coordinator send failures preserve inline error handling state', all(snippet in CHAIN_VIEW for snippet in [
    ...
    ('artifact chip labels are resolved from artifact metadata with id fallback', all(snippet in MARKDOWN_BODY for snippet in [
        'artifactsApi.endpoints.fetchArtifactMeta.initiate',
    ...
    ('artifacts panel uses project-scoped list api and viewer', all(snippet in PANEL for snippet in [
        ...
        'useListArtifactsQuery({ projectId, limit: 100 }',
    ...
    ('artifacts panel paste handler uploads clipboard_panel artifacts and relies on RTKQ invalidation', all(snippet in PANEL for snippet in [
    ...
    ('annotation list item actions expose debug ids for edit copy remove and daemon-backed comment editing', all(snippet in VIEWER for snippet in [
    ...
    ('artifact viewer versions and rollback use daemon-backed RTKQ hooks', all(snippet in VIEWER for snippet in [
    ...
    ('annotation utils keep the legacy localStorage key only for one-time migration and cleanup', all(snippet in ANNOTATIONS for snippet in [
        ...
        'export function listLegacyArtifactAnnotations(artifactId: string)',
        'export function clearLegacyArtifactAnnotations(artifactId: string)',
        'export function normalizeDaemonAnnotation(value: unknown',
    ])),
