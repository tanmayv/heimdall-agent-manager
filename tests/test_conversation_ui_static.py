...
checks = [
    ...
    ('new conversation page exposes mock option cards and integrated selectors', all(snippet in APP for snippet in [
        ...
        'data-debug-id="new-convo-hero-card"',
        'data-debug-id="new-convo-context-strip"',
    ...
    ('durable agent identity page separates identity defaults from concrete instances', all(snippet in APP for snippet in [
        ...
        '<code className="font-mono">agent_id = {durableId || \'—\'}</code>',
        'data-debug-id="agent-identity-edit-defaults-btn"',
        'data-debug-id="agent-identity-provider-tier"',
        'data-debug-id="agent-identity-memory-summary"',
        'Every instance below inherits these defaults.',
    ...
    ('global sidebar shows conversations before agents and expands show-more in place without passive scroll fetches', all(snippet in APP for snippet in [
    ...
    ]) and 'shouldLoadMoreFromScroll' not in APP and 'onScroll={(event) => { if (shouldLoadMoreFromScroll(event))' not in APP and APP[APP.index('function ConversationFocusedSidebar'):].index('<SidebarConversationSection') < APP[APP.index('function ConversationFocusedSidebar'):].index('<SidebarDurableAgentsSection')),
    ...
    ('global task chains/projects/agents surfaces expose create flows', all(snippet in APP for snippet in [
        ...
        'data-debug-id="agents-management-create-hint"',
        'showAgentList={false}',
    ...
    ('dedicated conversation thread page exposes mock-aligned debug ids', all(snippet in APP for snippet in [
        ...
        'data-debug-id="conversation-thread-summary-card"',
        'data-debug-id="conversation-thread-instance-label"',
        'data-debug-id="conversation-thread-affordance-strip"',
        'data-debug-id="conversation-thread-empty-state"',
    ...
    ('agent and conversation chat expose toggleable artifact side panels', all(snippet in APP for snippet in [
        ...
        'data-debug-id="agent-detail-chat-artifacts-toggle-btn"',
    ...
    ('chat and coordinator surfaces expose durable load-older pagination and view/WS reload hooks', all(snippet in APP + (ROOT / 'src/ui/store/chatSlice.ts').read_text(encoding='utf-8') + (ROOT / 'src/ui/store/chainViewSlice.ts').read_text(encoding='utf-8') for snippet in [
        ...
        'onRefreshChat: (agentId: string) => dispatch(fetchSelectedChat({ agentId }))',
    ...
    ('task panel paginates task comment logs and highlights completion comments', all(snippet in APP + (ROOT / 'src/ui/store/taskSlice.ts').read_text(encoding='utf-8') + (ROOT / 'src/ui/api/daemonApi.ts').read_text(encoding='utf-8') for snippet in [
        ...
        'loadTaskLogPage({ taskId, cursor }).unwrap().catch(() => undefined)',
