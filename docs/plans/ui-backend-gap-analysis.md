...
6. Global search/batch lookup for command palette and mention autocomplete (`GET /api/v1/search`, `POST /api/v1/batch/get`) is specified in the architecture doc but not currently registered in the Hub.
...
| Library and artifact viewer (`/library`, `/library/artifacts/:artifact_id`) | Filterable artifacts, content viewer, upload, download, rename/description, delete, versions, rollback, annotations. | **Partial.** Artifact list/create/detail/content/update/delete exist. | **Backend follow-up:** artifact list filters (`project_id`, `agent_id`, `agent_instance_id`, `chain_id`, `task_id`, `kind`), binary-safe content/download response, versions, rollback, annotations list/create/update/delete. Until implemented, UI-10 must either defer/disable versions/rollback/annotations or carry a backend dependency task. |
| Settings → Bridges | Bridge list/detail, enrollments, rename, revoke, capabilities, instances, path overrides. | **Ready/Partial.** Bridge list/detail/rename/revoke and enrollment create/list/revoke exist; project bridge paths exist. | **UI adapter:** build new endpoint module. **Backend follow-up:** token rotation is specified in architecture but not registered; capability refresh is indirect via Bridge runtime heartbeat only. |
| Settings → Projects | List/create/update project, default path, per-Bridge overrides, path validation, default `Conversations` project not deletable. | **Ready/Partial.** Project list/create/detail/update and bridge path override/validate exist. | **Backend follow-up:** explicit marker for default `Conversations` project (`is_default_conversations`, not deletable) if UI needs enforcement copy beyond name/slug convention; delete project is not currently registered and should remain absent for default project. |
...
| Command palette | Unified navigation/entity/action search and mention autocomplete. | **Backend follow-up.** No registered `GET /api/v1/search` or `POST /api/v1/batch/get`. | Add search endpoint from `docs/plans/hub-bridge-user-owned-architecture-and-api.md` §20A and the UI-17 gap matrix, plus optional batch lookup. Safe temporary assumption: client fan-out over list endpoints for small local beta only. |
| Responsive/mobile | Same data needs as desktop; no backend-specific extra routes. | **Ready.** | No backend gap. Ensure mobile uses the same route IDs and `/api/v1` adapters. |
| Data layer + user WS invalidation | RTK Query tags, one `/api/v1/user-ws`, resource invalidations for chat/task/agent/artifact/memory/project/bridge. | **Partial.** WebSocket route exists; source currently publishes `agent_instance` changes. | **Backend follow-up:** publish resource_changed for chats/messages, task chains/tasks/comments, artifacts, memories, projects, bridges/enrollments; define summary payloads or require refetch-only semantics. **UI adapter:** replace legacy tokenized WS URL with `/api/v1/user-ws`. |
| Legacy cleanup | Remove guide/workspace/attention/proxy/federation surfaces. | **Ready.** | Do not depend on backend legacy routes. Delete unused legacy endpoint modules only after `/api/v1` replacements exist. |
...
5. **UI-BE-5 Search/batch:** add `/api/v1/search` and optional `/api/v1/batch/get` for command palette and mention autocomplete.
