...
7. **UI-BE-7 WS publishers:** publish `resource_changed` for all resources used by RTK Query, not only `agent_instance`. **Partial (task/chain done):** `src/hub/transport/http/taskchain_handlers.odin` now publishes `resource_changed` for `task_chain` (create/update/publish/complete/member changes) and `task` (create/update/publish/status/cancel/comment/vote) via `event_bus` wired in `src/hub/app/wiring.odin`; the live UI handles the generic `resource_changed` envelope in `src/ui/api/wsInvalidation.ts` (invalidating the smallest `Task`/`TaskLog`/`TaskComments`/`ChainTasks`/`Chain`/`ChainList` tags) so task/chain views update live without a manual refresh. Verified by `tests/test_hub_phase10_real_smoke.py`. Remaining resources to cover: chats/messages, artifacts, memories, projects, bridges/enrollments.
