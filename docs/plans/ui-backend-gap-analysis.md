...
| Command palette | Unified navigation/entity/action search and mention autocomplete. | **Backend follow-up.** No registered `GET /api/v1/search` or `POST /api/v1/batch/get`. | `task-19f8ed3ac87` (UI-18) is the production entity-search prerequisite for Command palette task `task-19f8ec915a2`. It must add `/api/v1/search` from `docs/plans/hub-bridge-user-owned-architecture-and-api.md` §20A with search-as-you-type performance/indexing requirements; optional `/api/v1/batch/get` remains a separate optimization. Safe temporary assumption: client fan-out over list endpoints for small local beta only, bounded and documented, never unbounded typeahead. |
