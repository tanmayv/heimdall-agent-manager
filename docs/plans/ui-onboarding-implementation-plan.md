...
11. **DONE — Pane-native Bridge runtime (§3.12)** — Bridge now spawns the wrapper-supervisor with daemon-style flags, and the wrapper owns tmux pane creation, launches the agent argv as the foreground pane process with `HEIMDALL_BRIDGE_ENDPOINT` / `HEIMDALL_AGENT_TOKEN` / `HEIMDALL_AGENT_INSTANCE_ID` injected, reports `pane_id`, supervises pane lifecycle, removes stale launch records on pane exit, and shares daemon-free runtime helpers via `src/lib/agent_runtime`. Phase commits: `9c0d169`, `b5c8ec3`, `b25aaa1`.
12. **DONE — Wrapper-delivered message notifications (§3.13)** — Hub emits lightweight `notify_agent_message` commands, Bridge maintains wrapper notification subscriptions and pushes metadata-only frames, wrappers coalesce concise pane notifications, and `agent_to_agent` send/read plumbing is exposed through Hub, Bridge local endpoint, and `ham-ctl agent chat send-to-agent`. Phase commits: `9c0d169`, `b5c8ec3`, `b25aaa1`.
13. **DONE — Auth-gate/dev-proxy regression fixes (§3.11)** — dev-proxy has WebSocket pass-through for user WS, and AuthGate no longer wedges under React StrictMode remount because its single-flight refresh guard is effect-local and reset during cleanup. Phase commits: `130bcc3`, `eb69ae7`.
