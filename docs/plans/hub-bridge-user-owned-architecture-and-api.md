...
User API tokens are the bearer credentials used by machine/CLI clients
(`ham-ctl`, `ham-tui`, scripts) to act as a user. They are the counterpart to
browser trusted-proxy auth: the browser proves identity via the proxy; machine
clients present a user API token.

Rules:

- Tokens are stored **hashed**; the plaintext is shown **once** at creation.
- Tokens are presented only in the `Authorization` header (never query/body/URL).
- A token grants full-user access for its owner and is scoped by `owner_user_id`
  (fine-grained scopes are post-v1).
- Tokens are individually **revocable** and may have an optional expiry.
...
UserApiToken {
  ...
  owner_user_id: string
  label?: string
  token_hash: string          // never returned
  created_at: string
  last_used_at?: string
  expires_at?: string
  revoked_at?: string
