...
bootstrap_profile_guidance :: proc(profile, file_name: string) -> string {
	...
	strings.write_string(&builder, "tasks status --task-id <task-id> --chain-id <chain-id> --status in_progress --body \"Implementing now\"\n")
	strings.write_string(&builder, "tasks status --task-id <task-id> --chain-id <chain-id> --status review_ready --body \"All tests written and passing\"\n")
	strings.write_string(&builder, "\n")
