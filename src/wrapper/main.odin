...
memory_cli_guidance :: proc(agent_token: string) -> string {
	...
	strings.write_string(&builder, "` memory commands with your token for durable memory proposals and review. Approved active memory is what affects runtime behavior; pending, rejected, and archived proposals do not. Template targeting is a scope (template_id), not a memory type. Proposal reason/evidence are proposal-only review metadata and should not be copied into runtime memory bodies. Skill memories must use structured body text with at least name: and description:.\n")
	...
	strings.write_string(&builder, " --target-agent-id <agent_id> --target-project-id <project_id> --type fact|habit|episode|expertise|skill --title <title> --body <body> --reason <why> --evidence <task-or-source>; ")
