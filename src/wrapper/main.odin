...
bootstrap_profile_guidance :: proc(profile, file_name: string) -> string {
	...
	strings.write_string(&builder, "6. To request updates or redo an approved task, reviewers/users must add an unresolved comment, which automatically reverts the task to queued.\n")
	...
	strings.write_string(&builder, "```\n")
	strings.write_string(&builder, "# Claim your next queued task when you are free to work\n")
	...
	strings.write_string(&builder, "# `tasks later` moves the task back to queued; use `tasks next` when you want to resume it.\n")
	strings.write_string(&builder, "\n")
