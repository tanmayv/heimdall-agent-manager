...
task_service_create_task :: proc(cmd: Task_Create_Command) -> Task_Service_Result {
	if cmd.task_id != "" {
		return Task_Service_Result{ok = false, status_code = 400, message = `{"ok":false,"message":"task_id is generated by hub; omit task_id on create"}`}
	}
	if cmd.title == "" {
	...
	if cmd.assignee_agent_instance_id != "" || cmd.assignee_agent_id != "" {
		resolved, resolve_ok, resolve_msg := task_service_resolve_explicit_agent_target_for_chain(chain_id, cmd.project_id, cmd.assignee_agent_instance_id, cmd.assignee_agent_id, "assignee", "assignee", cmd.author_agent_instance_id)
	...
	reviewer_agent_instance_ref, normalized := task_normalize_user_reviewer(cmd.reviewer_agent_instance_id)
	reviewer_normalized = normalized
	if reviewer_agent_instance_ref != "" || cmd.reviewer_agent_id != "" {
		resolved, resolve_ok, resolve_msg := task_service_resolve_explicit_agent_target_for_chain(chain_id, cmd.project_id, reviewer_agent_instance_ref, cmd.reviewer_agent_id, "lgtm_required", "reviewer", cmd.author_agent_instance_id)
		if !resolve_ok do return Task_Service_Result{ok = false, status_code = 400, message = task_service_error_json(resolve_msg)}
...
task_service_create_chain :: proc(cmd: Task_Chain_Create_Command) -> Task_Service_Result {
	...
	if cmd.coordinator_agent_instance_id != "" || cmd.coordinator_agent_id != "" {
		resolved, resolve_ok, resolve_msg := task_service_resolve_explicit_agent_target_for_chain(chain_id, cmd.project_id, cmd.coordinator_agent_instance_id, cmd.coordinator_agent_id, "coordinator", "coordinator", cmd.author_agent_instance_id)
		if !resolve_ok do return Task_Service_Result{ok = false, status_code = 400, message = task_service_error_json(resolve_msg)}
		coordinator_agent_instance_id = resolved
	} else {
		coordinator_agent_id := agent_default_id_for_use("coordinator")
		if coordinator_agent_id == "" do coordinator_agent_id = "coordinator"
		resolved, resolve_ok, resolve_msg := task_service_resolve_agent_id_reference_for_new_chain(chain_id, cmd.project_id, coordinator_agent_id, "coordinator", cmd.author_agent_instance_id)
		if !resolve_ok do return Task_Service_Result{ok = false, status_code = 400, message = task_service_error_json(resolve_msg)}
		coordinator_agent_instance_id = resolved
	}
	...
	default_reviewer_normalized := false
	default_reviewer_agent_instance_ref, normalized := task_normalize_user_reviewer(cmd.default_reviewer_agent_instance_id)
	default_reviewer_normalized = normalized
	if default_reviewer_agent_instance_ref != "" || cmd.default_reviewer_agent_id != "" {
		resolved, resolve_ok, resolve_msg := task_service_resolve_explicit_agent_target_for_chain(chain_id, cmd.project_id, default_reviewer_agent_instance_ref, cmd.default_reviewer_agent_id, "lgtm_required", "default reviewer", cmd.author_agent_instance_id)
		if !resolve_ok do return Task_Service_Result{ok = false, status_code = 400, message = task_service_error_json(resolve_msg)}
		default_reviewer_agent_instance_id = resolved
	} else {
		default_reviewer_agent_id := agent_default_id_for_use("reviewer")
		if default_reviewer_agent_id != "" {
			if default_reviewer_agent_id == "user_proxy" || default_reviewer_agent_id == HUMAN_RECIPIENT_ID {
				default_reviewer_agent_instance_id, default_reviewer_normalized = task_normalize_user_reviewer(default_reviewer_agent_id)
			} else {
				resolved, resolve_ok, resolve_msg := task_service_resolve_agent_id_reference_for_new_chain(chain_id, cmd.project_id, default_reviewer_agent_id, "lgtm_required", cmd.author_agent_instance_id)
				if !resolve_ok do return Task_Service_Result{ok = false, status_code = 400, message = task_service_error_json(resolve_msg)}
				default_reviewer_agent_instance_id = resolved
			}
		}
...
task_service_resolve_agent_reference_for_chain :: proc(chain_id, project_id, agent_ref, participant_role, author: string) -> (string, bool, string) {
	_ = chain_id; _ = project_id; _ = participant_role; _ = author
	if agent_ref == "" do return "", false, "agent_instance_id is required"
	if agent_ref == "user_proxy" || agent_ref == HUMAN_RECIPIENT_ID do return agent_ref, true, "ok"
	if !valid_agent_instance_id(agent_ref) do return "", false, "invalid agent_instance_id; use agent_id for durable agent ids"
	if idx := agent_record_index_by_instance(agent_ref); idx >= 0 {
		if agent_instance_records[idx].archived_at_unix_ms != 0 do return "", false, "agent_instance_id is archived"
		return agent_ref, true, "ok"
	}
	return "", false, "agent_instance_id is unknown; use agent_id to materialize a durable agent"
}
...
task_service_validate_agent_id_reference :: proc(agent_id_ref: string) -> (string, bool, string) {
	agent_id := strings.trim_space(agent_id_ref)
	if agent_id == "" do return "", false, "agent_id is required"
	if agent_id == "user_proxy" || agent_id == HUMAN_RECIPIENT_ID do return "", false, "agent_id must be a durable agent id; use agent_instance_id for user_proxy/operator reviewers"
	if strings.index_byte(agent_id, '@') >= 0 do return "", false, "agent_id must not contain '@'; use agent_instance_id for concrete instances"
	if task_service_agent_ref_looks_indexed_slot(agent_id) do return "", false, "role slot is not a durable agent_id"
	if safe_agent_id_part(agent_id) != agent_id do return "", false, "invalid agent_id"
	return agent_id, true, "ok"
}

task_service_resolve_agent_id_reference_for_chain :: proc(chain_id, project_id, agent_id_ref, participant_role, author: string) -> (string, bool, string) {
	agent_id, valid, msg := task_service_validate_agent_id_reference(agent_id_ref)
	if !valid do return "", false, msg
	return task_service_create_concrete_instance_for_agent_id(agent_id, chain_id, project_id, participant_role, author)
}

task_service_resolve_agent_id_reference_for_new_chain :: proc(chain_id, project_id, agent_id_ref, participant_role, author: string) -> (string, bool, string) {
	return task_service_resolve_agent_id_reference_for_chain(chain_id, project_id, agent_id_ref, participant_role, author)
}

task_service_resolve_explicit_agent_target_for_chain :: proc(chain_id, project_id, agent_instance_id, agent_id, participant_role, target_label, author: string) -> (string, bool, string) {
	if agent_instance_id != "" && agent_id != "" do return "", false, fmt.tprintf("specify either %s agent_instance_id or %s agent_id, not both", target_label, target_label)
	if agent_id != "" do return task_service_resolve_agent_id_reference_for_chain(chain_id, project_id, agent_id, participant_role, author)
	if agent_instance_id != "" do return task_service_resolve_agent_reference_for_chain(chain_id, project_id, agent_instance_id, participant_role, author)
	return "", true, "ok"
}
...
task_service_assign :: proc(task_id, chain_id, agent_instance_id, author: string, agent_id: string = "") -> Task_Service_Result {
	return task_service_assign_command(Task_Assign_Command{task_id = task_id, chain_id = chain_id, agent_instance_id = agent_instance_id, agent_id = agent_id, author_agent_instance_id = author})
}

task_service_assign_command :: proc(cmd: Task_Assign_Command) -> Task_Service_Result {
	if cmd.task_id == "" {
		return Task_Service_Result{ok = false, status_code = 400, message = `{"ok":false,"message":"task assignment requires task_id"}`}
	}
	if cmd.agent_instance_id == "" && cmd.agent_id == "" {
		return Task_Service_Result{ok = false, status_code = 400, message = `{"ok":false,"message":"task assignment requires either agent_instance_id or agent_id"}`}
	}
	...
	agent_instance_id, resolve_ok, resolve_msg := task_service_resolve_explicit_agent_target_for_chain(state.chain_id, "", cmd.agent_instance_id, cmd.agent_id, "assignee", "assignee", cmd.author_agent_instance_id)
	if !resolve_ok do return Task_Service_Result{ok = false, status_code = 400, message = task_service_error_json(resolve_msg)}
...
task_service_add_participant :: proc(task_id, chain_id, agent_instance_id, role, author: string, agent_id: string = "") -> Task_Service_Result {
	return task_service_participant_command(Task_Participant_Command{task_id = task_id, chain_id = chain_id, agent_instance_id = agent_instance_id, agent_id = agent_id, role = role, author_agent_instance_id = author})
}

task_service_participant_command :: proc(cmd: Task_Participant_Command) -> Task_Service_Result {
	if cmd.task_id == "" || cmd.role == "" {
		return Task_Service_Result{ok = false, status_code = 400, message = `{"ok":false,"message":"participant requires task_id and role"}`}
	}
	if cmd.agent_instance_id == "" && cmd.agent_id == "" {
		return Task_Service_Result{ok = false, status_code = 400, message = `{"ok":false,"message":"participant requires either agent_instance_id or agent_id"}`}
	}
	agent_instance_ref := cmd.agent_instance_id
	reviewer_normalized := false
	if cmd.role == "lgtm_required" || cmd.role == "lgtm_optional" {
		agent_instance_ref, reviewer_normalized = task_normalize_user_reviewer(cmd.agent_instance_id)
	}
	switch cmd.role {
	...
	agent_instance_id, resolve_ok, resolve_msg := task_service_resolve_explicit_agent_target_for_chain(state.chain_id, "", agent_instance_ref, cmd.agent_id, cmd.role, "participant", cmd.author_agent_instance_id)
	if !resolve_ok do return Task_Service_Result{ok = false, status_code = 400, message = task_service_error_json(resolve_msg)}
...
task_service_update_chain :: proc(cmd: Task_Chain_Update_Command) -> Task_Service_Result {
	...
	if cmd.coordinator_agent_instance_id != "" || cmd.coordinator_agent_id != "" {
		resolved, resolve_ok, resolve_msg := task_service_resolve_explicit_agent_target_for_chain(cmd.chain_id, chain.project_id, cmd.coordinator_agent_instance_id, cmd.coordinator_agent_id, "coordinator", "coordinator", cmd.author_agent_instance_id)
		if !resolve_ok do return Task_Service_Result{ok = false, status_code = 400, message = task_service_error_json(resolve_msg)}
	...
	new_default_reviewer := old_default_reviewer
	default_reviewer_normalized := false
	new_default_reviewer_ref, normalized := task_normalize_user_reviewer(cmd.default_reviewer_agent_instance_id)
	default_reviewer_normalized = normalized
	if new_default_reviewer_ref != "" || cmd.default_reviewer_agent_id != "" {
		resolved, resolve_ok, resolve_msg := task_service_resolve_explicit_agent_target_for_chain(cmd.chain_id, chain.project_id, new_default_reviewer_ref, cmd.default_reviewer_agent_id, "lgtm_required", "default reviewer", cmd.author_agent_instance_id)
		if !resolve_ok do return Task_Service_Result{ok = false, status_code = 400, message = task_service_error_json(resolve_msg)}
	...
	if cmd.default_reviewer_agent_instance_id != "" || cmd.default_reviewer_agent_id != "" do event_default_reviewer = new_default_reviewer
	event_coordinator := ""
	if cmd.coordinator_agent_instance_id != "" || cmd.coordinator_agent_id != "" do event_coordinator = new_coordinator
	...
	task_notify_event(event)
	coordinator_changed := (cmd.coordinator_agent_instance_id != "" || cmd.coordinator_agent_id != "") && old_coordinator != new_coordinator
	default_reviewer_changed := (cmd.default_reviewer_agent_instance_id != "" || cmd.default_reviewer_agent_id != "") && old_default_reviewer != new_default_reviewer
