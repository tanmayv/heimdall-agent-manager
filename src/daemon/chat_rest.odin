...
handle_get_chats :: proc(client: net.TCP_Socket, ctx: ^Route_Context) {
	...
	// Daemon-authoritative ordering (most recent first, empty threads by
	// created/updated) + titles; mirrors chat_list_json used by list_chats.
	builder := strings.builder_make()
	strings.write_string(&builder, `{"chats":[`)
	rows := chat_list_build_rows(author)
