...
agent_store_init :: proc(data_dir: string) {
	...
	agent_id_seed_default_identities()
}
...
agent_store_apply_event :: proc(event: Agent_Instance_Event) -> bool {
	...
	// Derive the durable agent_id from the instance id prefix and ensure a backing
	// Agent_Id_Record exists. Generated chain/system instances carry project
	// context on the instance only and must not seed durable default projects.
	rec.agent_id = agent_id_from_instance_id(event.agent_instance_id)
	default_project_id := event.project_id
	if !agent_instance_may_seed_default_project(event.agent_instance_id, event.template_id) do default_project_id = ""
...
agent_instance_is_generated_chain :: proc(agent_instance_id: string) -> bool {
	if at := strings.index_byte(agent_instance_id, '@'); at >= 0 {
		suffix := agent_instance_id[at + 1:]
		return strings.contains(suffix, "-chain-") || strings.has_prefix(suffix, "chain-")
	}
	return false
}

agent_instance_may_seed_default_project :: proc(agent_instance_id, template_id: string) -> bool {
	if agent_instance_is_generated_chain(agent_instance_id) do return false
	if agent_system_id_configured(agent_instance_id) do return false
	return true
}
...
agent_template_event_clone :: proc(e: Agent_Template_Event) -> Agent_Template_Event {
	out := e
	out.event_id = strings.clone(e.event_id); out.template_id = strings.clone(e.template_id); out.display_name = strings.clone(e.display_name); out.description = strings.clone(e.description); out.persona = strings.clone(e.persona); out.instructions = strings.clone(e.instructions); out.parent_template_id = strings.clone(e.parent_template_id); out.default_provider_profile = strings.clone(e.default_provider_profile); out.bootstrap_defaults = strings.clone(e.bootstrap_defaults); out.suggested_model_tier = strings.clone(e.suggested_model_tier); out.author = strings.clone(e.author)
