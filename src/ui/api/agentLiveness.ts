...
export function agentHasLiveSession(agent: any): boolean {
  ...
  const runtimeStatus = String(agent.runtimeStatus || agent.runtime_status || '').toLowerCase();
  ...
  if (['stopped', 'failed', 'unreachable'].includes(runtimeStatus)) return false;
  if (['launching', 'starting', 'running', 'idle', 'busy', 'stopping'].includes(runtimeStatus)) return true;
  if (connected) return true;
  if (startup === 'stopping') return false;
