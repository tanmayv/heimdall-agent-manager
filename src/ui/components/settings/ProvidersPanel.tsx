...
function formFromProfile(profile: any): ProviderForm {
  ...
  return { ...emptyForm, name: String(profile.name || ''), enabled: Boolean(profile.enabled ?? true), command: asArray(profile.command), modelsFlag: String(profile.models?.flag || ''), modelsCheap: String(profile.models?.cheap || ''), modelsNormal: String(profile.models?.normal || ''), modelsSmart: String(profile.models?.smart || ''), promptFlags: asArray(profile.prompt_flags), yoloFlags: asArray(profile.yolo_flags), starterPrompt: String(profile.starter_prompt || ''), promptDelivery: String(profile.prompt_delivery || ''), skillDir: String(profile.skill_dir || ''), startupEnabled: Boolean(startup.enabled), startupProbeSeconds: String(startup.startup_probe_seconds ?? startup.probe_seconds ?? '20'), startupCaptureIntervalMs: String(startup.capture_interval_ms ?? '500'), startupBlockedPatterns: asLines(startup.blocked_patterns), startupAutoEnterPairs: Array.from({ length: Math.max(patterns.length, preKeys.length) }, (_, i) => ({ pattern: patterns[i] || '', preKey: preKeys[i] || '' })), startupUnknownIsBlocked: Boolean(startup.startup_unknown_is_blocked), startupReasonMappings: parseReasonMappings(startup.sanitized_reason_mapping), activityEnabled: Boolean(activity.enabled), activitySampleLines: String(activity.sample_line_count ?? '20'), activityIgnoreBottomLines: String(activity.ignore_bottom_lines ?? '0'), activityCheckIntervalSeconds: String(activity.check_interval_seconds ?? '2'), activityMinGapMs: String(activity.min_gap_ms ?? '250'), activityMaxGapMs: String(activity.max_gap_ms ?? '5000') };
}

function profileFromForm(form: ProviderForm): any {
  return { name: form.name.trim(), enabled: form.enabled, command: form.command, models: { flag: form.modelsFlag.trim(), cheap: form.modelsCheap.trim(), normal: form.modelsNormal.trim(), smart: form.modelsSmart.trim() }, prompt_flags: form.promptFlags, yolo_flags: form.yoloFlags, starter_prompt: form.starterPrompt, prompt_delivery: form.promptDelivery, skill_dir: form.skillDir.trim(), startup_detection: { enabled: form.startupEnabled, startup_probe_seconds: intValue(form.startupProbeSeconds, 20), capture_interval_ms: intValue(form.startupCaptureIntervalMs, 500), blocked_patterns: form.startupBlockedPatterns, auto_enter_patterns: form.startupAutoEnterPairs.map((pair) => pair.pattern), auto_enter_pre_keys: form.startupAutoEnterPairs.map((pair) => pair.preKey), startup_unknown_is_blocked: form.startupUnknownIsBlocked, sanitized_reason_mapping: form.startupReasonMappings.map((row) => `${row.key}=${row.reason}`) }, activity_detection: { enabled: form.activityEnabled, sample_line_count: intValue(form.activitySampleLines, 20), ignore_bottom_lines: intValue(form.activityIgnoreBottomLines, 0), check_interval_seconds: intValue(form.activityCheckIntervalSeconds, 2), min_gap_ms: intValue(form.activityMinGapMs, 250), max_gap_ms: intValue(form.activityMaxGapMs, 5000) } };
}
