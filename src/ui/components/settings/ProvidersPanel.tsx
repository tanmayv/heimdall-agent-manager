  ...
  return (
    <div className="w-full max-w-5xl space-y-6 text-left">
      ...
      <div className="flex justify-end">
        <a data-debug-id="providers-add-btn" href={shellHash(`/settings/providers/new?bridge=${encodeURIComponent(selectedId)}`)} aria-disabled={!selectedId || offline} className={`rounded-xl bg-sky-400 px-4 py-2 text-sm font-semibold text-black hover:bg-sky-300 ${!selectedId || offline ? 'pointer-events-none opacity-50' : ''}`}>＋ Add provider</a>
      </div>
            ...
            <div key={name} data-debug-id={`providers-provider-row-${name}`} className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <a data-debug-id={`providers-edit-btn-${name}`} href={shellHash(`/settings/providers/${encodeURIComponent(name)}/edit?bridge=${encodeURIComponent(selectedId)}`)} aria-disabled={offline} className={`rounded-lg border border-white/10 px-2.5 py-1 text-xs text-zinc-300 hover:bg-white/10 ${offline ? 'pointer-events-none opacity-50' : ''}`}>Edit</a>
...
export function ProviderEditorPage({ providerName = '' }: { providerName?: string }) {
  ...
  return (
    <div className="w-full max-w-5xl space-y-6 text-left">
      ...
      <div className="sticky bottom-0 flex justify-end gap-2 rounded-2xl border border-white/10 bg-[#0d0f14]/95 p-3 backdrop-blur"><a data-debug-id="providers-editor-cancel-btn" href={shellHash('/settings/providers')} className="rounded-xl bg-white/10 px-4 py-2 text-sm hover:bg-white/15">Cancel</a><button data-debug-id="providers-editor-save-btn" type="button" onClick={() => void saveProvider()} disabled={saving || offline || !form.name.trim()} className="rounded-xl bg-sky-400 px-4 py-2 text-sm font-semibold text-black hover:bg-sky-300 disabled:opacity-50">{saving ? 'Saving…' : 'Save provider'}</button></div>
    </div>
  );
}
...
export function ChipListInput({ prefix, label, placeholder, values, onChange }: { prefix: string; label: string; placeholder: string; values: string[]; onChange: (values: string[]) => void }) {
  ...
  return <div className="rounded-xl border border-white/10 bg-black/20 p-3"><div className="text-sm font-medium text-zinc-300">{label}</div><div className="mt-2 flex gap-2"><input data-debug-id={`${prefix}-chip-input`} value={draft} onChange={(e) => setDraft(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); add(); } }} placeholder={placeholder} className="min-w-0 flex-1 rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-sm outline-none focus:border-sky-400" /><button data-debug-id={`${prefix}-chip-add-btn`} type="button" onClick={add} className="rounded-xl bg-white/10 px-3 py-2 text-sm hover:bg-white/15">Add</button></div><div className="mt-2 flex flex-wrap gap-2">{values.map((value, index) => <span key={`${value}-${index}`} data-debug-id={`${prefix}-chip-${index}`} className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-xs text-zinc-200"><span>{value}</span><button data-debug-id={`${prefix}-chip-remove-btn-${index}`} type="button" onClick={() => onChange(values.filter((_, i) => i !== index))} className="text-zinc-400 hover:text-white">×</button></span>)}</div></div>;
}
...
function PairedListInput({ pairs, onChange }: { pairs: AutoEnterPair[]; onChange: (pairs: AutoEnterPair[]) => void }) {
  ...
  return <div className="rounded-xl border border-white/10 bg-black/20 p-3"><div className="text-sm font-medium text-zinc-300">Auto-enter pattern + pre-key pairs</div><div className="mt-2 grid gap-2 sm:grid-cols-[1fr_1fr_auto]"><input data-debug-id="providers-editor-startup-auto-enter-patterns-chip-input" value={pattern} onChange={(e) => setPattern(e.target.value)} placeholder="Yes, I trust this folder" className="rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-sm outline-none focus:border-sky-400" /><input data-debug-id="providers-editor-startup-auto-enter-pre-keys-chip-input" value={preKey} onChange={(e) => setPreKey(e.target.value)} placeholder="Enter" className="rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-sm outline-none focus:border-sky-400" /><button data-debug-id="providers-editor-startup-auto-enter-patterns-chip-add-btn" type="button" onClick={add} className="rounded-xl bg-white/10 px-3 py-2 text-sm hover:bg-white/15">Add</button></div><div className="mt-2 space-y-2">{pairs.map((pair, index) => <div key={index} data-debug-id={`providers-editor-startup-auto-enter-patterns-chip-${index}`} className="flex items-center justify-between gap-2 rounded-xl bg-white/10 px-3 py-2 text-xs text-zinc-200"><span className="min-w-0 truncate">{pair.pattern || '—'} → {pair.preKey || '—'}</span><button data-debug-id={`providers-editor-startup-auto-enter-patterns-chip-remove-btn-${index}`} type="button" onClick={() => onChange(pairs.filter((_, i) => i !== index))} className="text-zinc-400 hover:text-white">×</button></div>)}</div></div>;
}
...
function ReasonMappingInput({ rows, onChange }: { rows: ReasonMapping[]; onChange: (rows: ReasonMapping[]) => void }) {
  ...
  return <div className="rounded-xl border border-white/10 bg-black/20 p-3"><div className="text-sm font-medium text-zinc-300">Sanitized reason mapping</div><div className="mt-2 grid gap-2 sm:grid-cols-[1fr_1fr_auto]"><input data-debug-id="providers-editor-startup-reason-mapping-chip-input" value={keyValue} onChange={(e) => setKeyValue(e.target.value)} placeholder="permission_prompt" className="rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-sm outline-none focus:border-sky-400" /><input data-debug-id="providers-editor-startup-reason-mapping-reason-input" value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Waiting for trust confirmation" className="rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-sm outline-none focus:border-sky-400" /><button data-debug-id="providers-editor-startup-reason-mapping-chip-add-btn" type="button" onClick={add} className="rounded-xl bg-white/10 px-3 py-2 text-sm hover:bg-white/15">Add</button></div><div className="mt-2 space-y-2">{rows.map((row, index) => <div key={index} data-debug-id={`providers-editor-startup-reason-mapping-chip-${index}`} className="flex items-center justify-between gap-2 rounded-xl bg-white/10 px-3 py-2 text-xs text-zinc-200"><span className="min-w-0 truncate">{row.key || '—'} → {row.reason || '—'}</span><button data-debug-id={`providers-editor-startup-reason-mapping-chip-remove-btn-${index}`} type="button" onClick={() => onChange(rows.filter((_, i) => i !== index))} className="text-zinc-400 hover:text-white">×</button></div>)}</div></div>;
}
