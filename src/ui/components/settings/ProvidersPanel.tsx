  ...
  return (
    <div className="space-y-5 rounded-2xl border border-white/10 bg-white/[0.035] p-5">
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <label className="flex items-center gap-2 pt-6 text-sm text-zinc-300"><Checkbox data-debug-id="providers-enabled-toggle-editor" checked={form.enabled} onChange={(enabled) => setForm({ ...form, enabled })} /> Enabled</label>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <label className="block text-sm text-zinc-300">Bootstrap file name<Input data-debug-id="providers-editor-bootstrap-file-name-input" value={form.bootstrapFileName} onChange={(value) => setForm({ ...form, bootstrapFileName: value })} placeholder="CLAUDE.md for claude, AGENTS.md otherwise (leave blank to keep default)" width="full" className="mt-1 min-h-[44px]" /><span className="mt-1 block text-xs text-zinc-500">The single bootstrap file this provider&apos;s agent reads on startup. Leave blank to keep the profile default (CLAUDE.md for the claude profile, AGENTS.md otherwise). Changing it regenerates the file under the new name and cleans up the old one on the next launch.</span></label>
      ...
      <div data-debug-id="providers-editor-startup-help" className="rounded-xl border border-white/10 bg-black/20 p-3 text-xs text-zinc-400">
        ...
        <ul className="mt-2 space-y-1 text-zinc-500">
          <li><span className="text-zinc-300">Startup detection</span> — master on/off switch for this pane-watching behavior.</li>
          <li><span className="text-zinc-300">Unknown startup is blocked</span> — if the pane shows something unrecognized, treat it as &quot;stuck, needs a human&quot; instead of assuming it&apos;s fine.</li>
          <li><span className="text-zinc-300">Startup probe seconds</span> — how long to keep watching the pane for prompts after launch.</li>
          <li><span className="text-zinc-300">Capture interval ms</span> — how often to sample/re-read the pane while probing.</li>
          <li><span className="text-zinc-300">Blocked patterns</span> — text that means &quot;stuck, needs a human&quot;; if seen, the agent is marked blocked rather than auto-answered.</li>
          <li><span className="text-zinc-300">Auto-enter pattern + pre-key pairs</span> — when the pane matches a Pattern, send the Pre-key(s) and then Enter (see the section below for details and examples).</li>
          <li><span className="text-zinc-300">Sanitized reason mapping</span> — maps an internal key to a human-readable &quot;why blocked&quot; reason shown in the UI.</li>
        </ul>
      </div>
      <div className="grid gap-4 sm:grid-cols-2"><label className="flex items-center gap-2 text-sm text-zinc-300"><Checkbox data-debug-id="providers-editor-startup-enabled-checkbox" checked={form.startupEnabled} onChange={(startupEnabled) => setForm({ ...form, startupEnabled })} /> Startup detection</label><label className="flex items-center gap-2 text-sm text-zinc-300"><Checkbox data-debug-id="providers-editor-startup-unknown-blocked-checkbox" checked={form.startupUnknownIsBlocked} onChange={(startupUnknownIsBlocked) => setForm({ ...form, startupUnknownIsBlocked })} /> Unknown startup is blocked</label><NumberInput id="providers-editor-startup-probe-input" label="Startup probe seconds" value={form.startupProbeSeconds} onChange={(startupProbeSeconds) => setForm({ ...form, startupProbeSeconds })} placeholder="20" min={0} /><NumberInput id="providers-editor-startup-capture-interval-input" label="Capture interval ms" value={form.startupCaptureIntervalMs} onChange={(startupCaptureIntervalMs) => setForm({ ...form, startupCaptureIntervalMs })} placeholder="500" min={0} /></div>
...
function PairedListInput({ pairs, onChange }: { pairs: AutoEnterPair[]; onChange: (pairs: AutoEnterPair[]) => void }) {
  ...
  return <div className="rounded-xl border border-white/10 bg-black/20 p-3"><div className="text-sm font-medium text-zinc-300">Auto-enter pattern + pre-key pairs</div><p data-debug-id="providers-editor-startup-auto-enter-help" className="mt-1 text-xs text-zinc-400">When the pane text matches the <span className="text-zinc-300">Pattern</span>, the wrapper first sends the <span className="text-zinc-300">Pre-key(s)</span>, waits ~150ms, then <span className="text-zinc-300">always sends Enter</span>. So the Pre-key field is what to press BEFORE the automatic Enter — leave it empty to just press Enter.</p><p className="mt-1 text-xs text-zinc-500">The Pre-key field accepts multiple tmux key names separated by <span className="text-zinc-300">spaces</span>; each is sent as a separate keypress in order, before Enter. Example: to move the selection down and then confirm, enter <span className="text-zinc-300">Down</span> as the pre-key (Enter is sent automatically). For two steps: <span className="text-zinc-300">Down Down</span>. Other tmux keys work too, e.g. <span className="text-zinc-300">Tab</span>, <span className="text-zinc-300">Up</span>, <span className="text-zinc-300">Enter</span>, <span className="text-zinc-300">Space</span>.</p><div className="mt-2 grid gap-2 sm:grid-cols-[1fr_1fr_auto]"><Input data-debug-id="providers-editor-startup-auto-enter-patterns-chip-input" value={pattern} onChange={setPattern} placeholder="Yes, I trust this folder" width="full" className="min-h-[44px] min-w-0" /><Input data-debug-id="providers-editor-startup-auto-enter-pre-keys-chip-input" value={preKey} onChange={setPreKey} placeholder="Enter" width="full" className="min-h-[44px] min-w-0" /><Button variant="secondary" data-debug-id="providers-editor-startup-auto-enter-patterns-chip-add-btn" onClick={add} className="min-h-[44px]">Add</Button></div><div className="mt-2 space-y-2">{pairs.map((pair, index) => <div key={index} data-debug-id={`providers-editor-startup-auto-enter-patterns-chip-${index}`} className="flex items-center justify-between gap-2 rounded-xl bg-white/10 px-3 py-2 text-xs text-zinc-200"><span className="min-w-0 break-all">{pair.pattern || '—'} → {pair.preKey || '—'}</span><button data-debug-id={`providers-editor-startup-auto-enter-patterns-chip-remove-btn-${index}`} type="button" onClick={() => onChange(pairs.filter((_, i) => i !== index))} className="min-h-[32px] min-w-[32px] text-zinc-400 hover:text-white">×</button></div>)}</div></div>;
}
...
function ReasonMappingInput({ rows, onChange }: { rows: ReasonMapping[]; onChange: (rows: ReasonMapping[]) => void }) {
  ...
  return <div className="rounded-xl border border-white/10 bg-black/20 p-3"><div className="text-sm font-medium text-zinc-300">Sanitized reason mapping</div><div className="mt-2 grid gap-2 sm:grid-cols-[1fr_1fr_auto]"><Input data-debug-id="providers-editor-startup-reason-mapping-chip-input" value={keyValue} onChange={setKeyValue} placeholder="permission_prompt" width="full" className="min-h-[44px] min-w-0" /><Input data-debug-id="providers-editor-startup-reason-mapping-reason-input" value={reason} onChange={setReason} placeholder="Waiting for trust confirmation" width="full" className="min-h-[44px] min-w-0" /><Button variant="secondary" data-debug-id="providers-editor-startup-reason-mapping-chip-add-btn" onClick={add} className="min-h-[44px]">Add</Button></div><div className="mt-2 space-y-2">{rows.map((row, index) => <div key={index} data-debug-id={`providers-editor-startup-reason-mapping-chip-${index}`} className="flex items-center justify-between gap-2 rounded-xl bg-white/10 px-3 py-2 text-xs text-zinc-200"><span className="min-w-0 break-all">{row.key || '—'} → {row.reason || '—'}</span><button data-debug-id={`providers-editor-startup-reason-mapping-chip-remove-btn-${index}`} type="button" onClick={() => onChange(rows.filter((_, i) => i !== index))} className="min-h-[32px] min-w-[32px] text-zinc-400 hover:text-white">×</button></div>)}</div></div>;
}
