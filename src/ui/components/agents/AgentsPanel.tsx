    ...
    <div className="w-full max-w-5xl space-y-6 text-left">
      ...
      <form onSubmit={handleSubmit} className="rounded-2xl border border-white/10 bg-white/[0.035] p-4 sm:p-5">
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-3 sm:col-span-2"><Button variant="ghost" data-debug-id="agents-create-instructions-toggle-btn" onClick={() => state.setShowInstructions(!state.showInstructions)} className="min-h-[44px]">{state.showInstructions ? 'Hide' : 'Customize'} instructions</Button>{state.showInstructions ? <textarea data-debug-id="agents-create-instructions-input" value={state.instructions} onChange={(e) => state.setInstructions(e.target.value)} placeholder="Optional additions layered on the selected template." className="mt-2 h-28 w-full rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-sm outline-none focus:border-sky-400" /> : null}</div>
