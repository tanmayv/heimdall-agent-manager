...
export function NewAgentPage() {
  ...
  return (
    <div className="w-full max-w-5xl space-y-6 text-left">
      ...
      <form onSubmit={handleSubmit} className="rounded-2xl border border-white/10 bg-white/[0.035] p-4 pb-24 sm:p-5 sm:pb-5">
        ...
        <div className="mt-4 rounded-xl border border-white/10 bg-black/20 px-3 py-2 text-xs text-zinc-500">Will enable <span className="text-zinc-200">{state.supportPreviewCount}</span> Bridge{state.supportPreviewCount === 1 ? '' : 's'} that support the selected provider/tier.</div>
        <div className="sticky bottom-20 z-10 mt-5 flex flex-col-reverse gap-2 rounded-2xl border border-white/10 bg-[#0d0f14]/95 p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] backdrop-blur md:bottom-0 sm:flex-row sm:justify-end">
