...
function MemoryProposalForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="framer-card-xl mx-auto max-w-4xl space-y-4 p-5">
      ...
      <textarea data-debug-id="proposal-body" value={form.body} onChange={(event) => updateForm({ body: event.target.value })} onKeyDown={handleKeyDownCtrlW} placeholder="memory body" className="framer-input min-h-36 w-full px-3 py-2 resize-y" disabled={form.proposalAction === 'archive' || form.proposalAction === 'rollback'} />
      <textarea data-debug-id="proposal-reason" value={form.reason} onChange={(event) => updateForm({ reason: event.target.value })} onKeyDown={handleKeyDownCtrlW} placeholder="required reason" className="framer-input min-h-20 w-full px-3 py-2 resize-y" required />
      <textarea data-debug-id="proposal-evidence" value={form.evidence} onChange={(event) => updateForm({ evidence: event.target.value })} onKeyDown={handleKeyDownCtrlW} placeholder="required evidence" className="framer-input min-h-20 w-full px-3 py-2 resize-y" required />
