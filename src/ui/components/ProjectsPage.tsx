        ...
        {page === 'create' ? (
          <form onSubmit={submitCreate} className="framer-card-xl mx-auto max-w-4xl space-y-4 p-5">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between"><p className="framer-topline">Initial anchors</p><button type="button" data-debug-id="create-project-add-anchor-btn" onClick={() => setForm((current) => ({ ...current, anchors: [...current.anchors, { ...blankAnchor }] }))} className="framer-pill-secondary px-3 py-2 text-xs">+ Anchor</button></div>
              {form.anchors.map((anchor, index) => (
                <div key={index} className="framer-card grid grid-cols-[0.65fr_1fr_1fr_auto_auto] gap-2 p-3">
                  ...
                  {anchor.type.trim().toLowerCase() === 'directory' ? <button type="button" data-debug-id={`create-anchor-browse-btn-${index}`} onClick={() => chooseDirectoryAnchor('create', index)} className="framer-pill-secondary px-3 py-2 text-xs">Browse…</button> : null}
            ...
            <div className="flex justify-end gap-2"><button type="button" data-debug-id="create-project-cancel-btn" onClick={() => setPage('list')} className="framer-pill-secondary">Cancel</button><button data-debug-id="create-project-submit-btn" disabled={!form.name.trim() || mutating} className="framer-pill disabled:opacity-40">{mutating ? 'Creating…' : 'Create project'}</button></div>
          </form>
        ) : (
              ...
              {selectedProject ? (
                <>
                  ...
                  <form onSubmit={submitDetail} className="mt-5 space-y-4">
                    <input data-debug-id="detail-project-name" value={detailForm.name} onChange={(event) => setDetailForm({ ...detailForm, name: event.target.value })} placeholder="Project name" className="framer-input w-full px-3 py-2 text-sm" required />
                    <textarea data-debug-id="detail-project-description" value={detailForm.description} onChange={(event) => setDetailForm({ ...detailForm, description: event.target.value })} placeholder="Project description" className="framer-input min-h-24 w-full px-3 py-2 text-sm" />
                    <div className="space-y-3">
                      <div className="flex items-center justify-between"><p className="framer-topline">Anchors</p><button type="button" data-debug-id="detail-project-add-anchor-btn" onClick={() => setDetailForm((current) => ({ ...current, anchors: [...current.anchors, { ...blankAnchor }] }))} className="framer-pill-secondary px-3 py-2 text-xs">+ Anchor</button></div>
                      {detailForm.anchors.map((anchor, index) => (
                        <div key={index} className="framer-card grid grid-cols-[0.65fr_1fr_1fr_auto_auto] gap-2 p-3">
                          <input data-debug-id={`detail-anchor-type-${index}`} value={anchor.type} onChange={(event) => updateDetailAnchor(index, { type: event.target.value })} placeholder="type" list="anchor-types" className="framer-input px-3 py-2 text-sm" />
                          <input data-debug-id={`detail-anchor-value-${index}`} value={anchor.value} onChange={(event) => updateDetailAnchor(index, { value: event.target.value })} placeholder="value" className="framer-input px-3 py-2 text-sm" />
                          <input data-debug-id={`detail-anchor-note-${index}`} value={anchor.note} onChange={(event) => updateDetailAnchor(index, { note: event.target.value })} placeholder="note" className="framer-input px-3 py-2 text-sm" />
                          {anchor.type.trim().toLowerCase() === 'directory' ? <button type="button" data-debug-id={`detail-anchor-browse-btn-${index}`} onClick={() => chooseDirectoryAnchor('detail', index)} className="framer-pill-secondary px-3 py-2 text-xs">Browse…</button> : null}
                          <button type="button" data-debug-id={`detail-anchor-remove-btn-${index}`} onClick={() => removeDetailAnchor(index)} className="framer-pill-secondary px-3 py-2 text-xs">Remove</button>
                        </div>
                      ))}
                    </div>
                    <div className="flex justify-end"><button data-debug-id="detail-project-save-btn" disabled={!detailForm.name.trim() || mutating} className="framer-pill disabled:opacity-40">{mutating ? 'Saving…' : 'Save project'}</button></div>
                  </form>
                  ...
                  <section className="mt-6 space-y-4 border-t border-[var(--fd-hairline)] pt-5">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <button type="button" data-debug-id="project-agents-refresh-btn" onClick={() => refreshProjectAgents(selectedProjectId)} className="framer-pill-secondary px-3 py-2 text-xs">Refresh</button>
                    </div>
                    ...
                    <div className="space-y-2">
                      {projectAgents.length ? projectAgents.map((agent) => (
                        <div key={agentRecordId(agent) || agentInstanceId(agent)} className="framer-card flex items-start justify-between gap-3 p-3">
                          ...
                          <button type="button" data-debug-id="project-agent-remove-btn" onClick={() => removeProjectAgent(agent)} disabled={agentBusy} className="framer-pill-secondary px-3 py-2 text-xs disabled:opacity-40">Remove</button>
                        </div>
