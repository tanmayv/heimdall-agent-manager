...
function AgentSelect({ value, onChange, agents, placeholder = 'Select agent' }: { value: string; onChange: (value: string) => void; agents: string[]; placeholder?: string }) {
  return (
    <select value={value} onChange={(event) => onChange(event.target.value)} className="framer-input w-full px-3 py-2 text-sm">
    ...
    return (
      <header className="animate-float-in border-b border-[var(--fd-hairline)] bg-[var(--fd-surface-2)] px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {page !== 'overview' && <button type="button" onClick={back} className="framer-pill-secondary">Back</button>}
          {page === 'chain' ? (
            <button type="button" onClick={() => openCreateTask('subtask')} className="framer-pill bg-white">+ Add Task</button>
          ) : (
            <>
              <button type="button" onClick={openCreateChain} className="framer-pill-secondary">+ Chain</button>
              <button type="button" onClick={() => openCreateTask('root')} className="framer-pill bg-white">+ Task</button>
            </>
          )}
          <button type="button" onClick={() => dispatch(refreshTaskBoard())} disabled={!session.connected || loading} className="framer-pill-secondary disabled:cursor-not-allowed disabled:opacity-40">{loading ? 'Refreshing…' : 'Refresh'}</button>
        </div>
      </header>
    );
  }
  ...
  function renderOverview() {
    ...
    chainIds.forEach((chainId) => {
      ...
      chainsByBucket[bucket].push(
        <button key={chainId} type="button" draggable onDragStart={() => handleDragStart('chain', chainId, bucket)} onDragEnd={() => setDraggedItem(null)} onClick={() => openChain(chainId)} className="framer-card w-full cursor-grab p-3 text-left transition-all hover:border-[var(--fd-accent-blue)]/50 active:cursor-grabbing">
  ...
  function renderChainView() {
    ...
    taskIds.forEach((taskId) => {
      ...
      tasksByBucket[bucket].push(
        <button key={taskId} type="button" draggable onDragStart={() => handleDragStart('task', taskId, bucket)} onDragEnd={() => setDraggedItem(null)} onClick={() => openTask(taskId)} className="framer-card w-full cursor-grab p-3 text-left transition-all hover:border-[var(--fd-accent-blue)]/50 active:cursor-grabbing">
        ...
        <section className="min-h-0 flex-1 overflow-y-auto p-4">
          <div className="framer-card-xl mb-4 p-4">
            ...
            <form onSubmit={handleChainMetadata} className="mt-4 grid gap-3">
              <input value={chainForm.title} onChange={(event) => setChainForm({ ...chainForm, title: event.target.value })} placeholder="Chain title" className="framer-input px-3 py-2 text-sm" />
              <textarea value={chainForm.description} onChange={(event) => setChainForm({ ...chainForm, description: event.target.value })} placeholder="Chain description" rows={2} className="framer-input resize-none px-3 py-2 text-sm" />
              <div className="grid grid-cols-2 gap-3"><AgentSelect value={chainForm.coordinatorAgentInstanceId} onChange={(value) => setChainForm({ ...chainForm, coordinatorAgentInstanceId: value })} agents={agentOptions} placeholder="No coordinator" /><AgentSelect value={chainForm.defaultReviewerAgentInstanceId} onChange={(value) => setChainForm({ ...chainForm, defaultReviewerAgentInstanceId: value })} agents={agentOptions} placeholder="No default reviewer" /></div>
              <textarea value={chainForm.finalSummary} onChange={(event) => setChainForm({ ...chainForm, finalSummary: event.target.value })} placeholder="Final summary" rows={2} className="framer-input resize-none px-3 py-2 text-sm" />
              <div className="flex flex-wrap gap-2"><button disabled={!canMutate} className="framer-pill bg-white disabled:opacity-40">Save metadata</button></div>
            </form>
            <form onSubmit={handleChainStatus} className="mt-3 grid grid-cols-[0.55fr_1fr_auto] gap-2"><input value={chainStatusForm.status} onChange={(event) => setChainStatusForm({ ...chainStatusForm, status: event.target.value })} placeholder="status" className="framer-input px-3 py-2 text-sm" /><input value={chainStatusForm.finalSummary} onChange={(event) => setChainStatusForm({ ...chainStatusForm, finalSummary: event.target.value })} placeholder="final summary" className="framer-input px-3 py-2 text-sm" /><button disabled={!canMutate || !chainStatusForm.status.trim()} className="framer-pill bg-white disabled:opacity-40">Set status</button></form>
          </div>
          <div className="grid min-h-[420px] grid-cols-4 gap-4">
            {STATUS_COLUMNS.map((column) => renderColumn(column, [
              <button key={`${column.id}-add`} type="button" onClick={() => openCreateTask('subtask')} className="framer-card mb-2 w-full border-dashed p-3 text-left text-sm text-[var(--fd-accent-blue)] hover:border-[var(--fd-accent-blue)]/50">+ Add task to chain</button>,
      ...
      <>
        ...
        <section className="min-h-0 flex-1 overflow-y-auto p-6">
          <form onSubmit={handleCreateChain} className="framer-card-xl mx-auto max-w-3xl p-6">
            ...
            <input value={createChainForm.title} onChange={(event) => setCreateChainForm({ ...createChainForm, title: event.target.value })} placeholder="Chain title" className="framer-input mt-5 w-full px-3 py-3 text-sm" />
            <input value={createChainForm.chainId} onChange={(event) => setCreateChainForm({ ...createChainForm, chainId: event.target.value })} placeholder="Optional chain id" className="framer-input mt-2 w-full px-3 py-2 text-sm" />
            <textarea value={createChainForm.description} onChange={(event) => setCreateChainForm({ ...createChainForm, description: event.target.value })} placeholder="Chain-level description" rows={7} className="framer-input mt-2 w-full resize-none px-3 py-3 text-sm" />
            <div className="mt-3 grid grid-cols-2 gap-3">
              <AgentSelect value={createChainForm.coordinatorAgentInstanceId} onChange={(value) => setCreateChainForm({ ...createChainForm, coordinatorAgentInstanceId: value })} agents={agentOptions} placeholder="No coordinator" />
              <AgentSelect value={createChainForm.defaultReviewerAgentInstanceId} onChange={(value) => setCreateChainForm({ ...createChainForm, defaultReviewerAgentInstanceId: value })} agents={agentOptions} placeholder="No reviewer" />
            </div>
            <div className="mt-6 flex justify-end gap-3">
              <button type="button" onClick={back} className="framer-pill-secondary">Cancel</button>
              <button disabled={!canMutate || !createChainForm.title.trim()} className="framer-pill bg-white disabled:opacity-40">Create chain</button>
            </div>
          </form>
        </section>
      </>
        ...
        <section className="min-h-0 flex-1 overflow-y-auto p-6">
          <form onSubmit={handleCreateTask} className="framer-card-xl mx-auto max-w-3xl p-6">
            ...
            <div className="mt-5 grid grid-cols-3 gap-3">
              <select value={createForm.mode} onChange={(event) => setCreateForm({ ...createForm, mode: event.target.value })} className="framer-input px-3 py-2 text-sm"><option value="root">Root task + chain</option><option value="subtask">Task in selected chain</option><option value="standalone">Standalone task</option></select>
              <input value={createForm.priority} onChange={(event) => setCreateForm({ ...createForm, priority: event.target.value })} placeholder="priority" className="framer-input px-3 py-2 text-sm" />
              <input value={createForm.status} onChange={(event) => setCreateForm({ ...createForm, status: event.target.value })} placeholder="status" className="framer-input px-3 py-2 text-sm" />
            </div>
            ...
            <input value={createForm.title} onChange={(event) => setCreateForm({ ...createForm, title: event.target.value })} placeholder="Task title" className="framer-input mt-4 w-full px-3 py-3 text-sm" />
            <textarea value={createForm.description} onChange={(event) => setCreateForm({ ...createForm, description: event.target.value })} placeholder="Task-specific description" rows={7} className="framer-input mt-2 w-full resize-none px-3 py-3 text-sm" />
            <div className="mt-3 grid grid-cols-3 gap-3">
              <AgentSelect value={createForm.assigneeAgentInstanceId} onChange={(value) => setCreateForm({ ...createForm, assigneeAgentInstanceId: value })} agents={agentOptions} placeholder="No assignee" />
              <AgentSelect value={createForm.reviewerAgentInstanceId} onChange={(value) => setCreateForm({ ...createForm, reviewerAgentInstanceId: value })} agents={agentOptions} placeholder="No reviewer" />
              <AgentSelect value={createForm.coordinatorAgentInstanceId} onChange={(value) => setCreateForm({ ...createForm, coordinatorAgentInstanceId: value })} agents={agentOptions} placeholder="No coordinator" />
            </div>
            <div className="mt-6 flex justify-end gap-3">
              <button type="button" onClick={back} className="framer-pill-secondary">Cancel</button>
              <button disabled={!canMutate || !createForm.title.trim() || (createForm.mode === 'subtask' && !selectedChainId)} className="framer-pill bg-white disabled:opacity-40">Create task</button>
            </div>
          </form>
        </section>
      </>
      ...
      <>
        ...
        <section className="min-h-0 flex-1 overflow-y-auto p-6">
          <div className="mx-auto max-w-4xl space-y-4">
            ...
            <div className="framer-card p-4"><p className="framer-topline">Mutations</p><form onSubmit={handleComment} className="mt-3 flex gap-2"><input value={commentBody} onChange={(event) => setCommentBody(event.target.value)} placeholder="Add comment" className="framer-input min-w-0 flex-1 px-3 py-2 text-sm" /><button disabled={!canMutate || !commentBody.trim()} className="framer-pill bg-white disabled:opacity-40">Comment</button></form><form onSubmit={handleStatus} className="mt-2 grid grid-cols-[0.55fr_1fr_auto] gap-2"><input value={statusForm.status} onChange={(event) => setStatusForm({ ...statusForm, status: event.target.value })} placeholder="status" className="framer-input px-3 py-2 text-sm" /><input value={statusForm.body} onChange={(event) => setStatusForm({ ...statusForm, body: event.target.value })} placeholder="status note" className="framer-input px-3 py-2 text-sm" /><button disabled={!canMutate || !statusForm.status.trim() || !statusForm.body.trim()} className="framer-pill bg-white disabled:opacity-40">Update</button></form><form onSubmit={handleAssign} className="mt-2 grid grid-cols-[1fr_auto] gap-2"><AgentSelect value={assignmentAgent} onChange={setAssignmentAgent} agents={agentOptions} placeholder="New assignee" /><button disabled={!canMutate || !assignmentAgent.trim()} className="framer-pill bg-white disabled:opacity-40">Assign</button></form><form onSubmit={handleParticipant} className="mt-2 grid grid-cols-[1fr_0.55fr_auto] gap-2"><AgentSelect value={participantForm.agentInstanceId} onChange={(value) => setParticipantForm({ ...participantForm, agentInstanceId: value })} agents={agentOptions} placeholder="Participant" /><select value={participantForm.role} onChange={(event) => setParticipantForm({ ...participantForm, role: event.target.value })} className="framer-input px-3 py-2 text-sm"><option value="reviewer">reviewer</option><option value="coordinator">coordinator</option><option value="verifier">verifier</option><option value="observer">observer</option><option value="specialist">specialist</option><option value="assignee">assignee</option></select><button disabled={!canMutate || !participantForm.agentInstanceId.trim()} className="framer-pill bg-white disabled:opacity-40">Add</button></form></div>
