...
ctl_tasks :: proc(daemon_url, action: string, args: []string) {
	...
	if action == "create" && task_id_value != "" {
		fmt.println(`{"ok":false,"message":"task_id is generated by hub; omit --task-id on create"}`)
		return
	}
	...
	case "create":
		...
		strings.write_string(&body, `,"assignee_agent_instance_id":"`);    json_write_string(&body, option_value(args, "--assignee-agent-instance-id", option_value(args, "--assignee-instance-id", option_value(args, "--assignee", "")))); strings.write_string(&body, `"`)
		strings.write_string(&body, `,"assignee_agent_id":"`);             json_write_string(&body, option_value(args, "--assignee-agent-id", "")); strings.write_string(&body, `"`)
		strings.write_string(&body, `,"reviewer_agent_instance_id":"`);    json_write_string(&body, option_value(args, "--reviewer-agent-instance-id", option_value(args, "--reviewer-instance-id", option_value(args, "--reviewer", "")))); strings.write_string(&body, `"`)
		strings.write_string(&body, `,"reviewer_agent_id":"`);             json_write_string(&body, option_value(args, "--reviewer-agent-id", "")); strings.write_string(&body, `"`)
	...
	case "assign":
		...
		strings.write_string(&body, `,"agent_id":"`);          json_write_string(&body, option_value(args, "--agent-id", ""));          strings.write_string(&body, `"`)
	case "participant":
...
ctl_task_chains :: proc(daemon_url, action: string, args: []string) {
	...
	if action == "create" {
		...
		strings.write_string(&body, `,"coordinator_agent_id":"`); json_write_string(&body, option_value(args, "--coordinator-agent-id", "")); strings.write_string(&body, `"`)
		strings.write_string(&body, `,"default_reviewer_agent_instance_id":"`); json_write_string(&body, option_value(args, "--reviewer-agent-instance-id", option_value(args, "--reviewer", ""))); strings.write_string(&body, `"`)
		strings.write_string(&body, `,"default_reviewer_agent_id":"`); json_write_string(&body, option_value(args, "--reviewer-agent-id", "")); strings.write_string(&body, `"`)
...
print_usage :: proc(config_path, daemon_url: string) {
	...
	fmt.println("  tasks create --token <token> --title <title> [--description <text>] [--assignee-agent-instance-id <id>|--assignee-agent-id <agent_id>] [--reviewer-agent-instance-id <id>|--reviewer-agent-id <agent_id>] [--status planning] [--depends-on <task-id,...>] [--chain-id <id>|--standalone]")
	fmt.println("    task_id is always generated by hub; root tasks auto-create a planning chain unless --standalone is set")
	fmt.println("    agent_instance_id flags target existing concrete instances only; agent_id flags materialize a new instance from a durable id")
	...
	fmt.println("  tasks assign --token <token> --task-id <id> (--agent-instance-id <id>|--agent-id <agent_id>)")
	fmt.println("  tasks participant --token <token> --task-id <id> (--agent-instance-id <id>|--agent-id <agent_id>) --role <assignee|lgtm_required|lgtm_optional|coordinator|subscriber>")
	...
	fmt.println("  task-chains create --token <token> [--project-id <id>] [--title <title>] [--description|--goal <text>] [--vcs|--no-vcs] [--reviewer-agent-instance-id <id>|--reviewer-agent-id <agent_id>] [--coordinator-agent-instance-id <id>|--coordinator-agent-id <agent_id>]")
	fmt.println("  task-chains activate --token <token> --chain-id <id>    (planning → in_progress, tasks begin auto-promoting)")
	fmt.println("  task-chains update --token <token> --chain-id <id> [--title <text>] [--description <text>] [--coordinator-agent-instance-id <id>|--coordinator-agent-id <agent_id>] [--reviewer-agent-instance-id <id>|--reviewer-agent-id <agent_id>]")
