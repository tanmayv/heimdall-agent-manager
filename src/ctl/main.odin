...
ctl_tasks :: proc(daemon_url, action: string, args: []string) {
	token := option_value(args, "--token", "")
	if token == "" { fmt.println("usage: ham-ctl tasks <create|list|next|show|log|comment|status|assign|participant|review> --token <token> ..."); return }
	path := ""
	...
	case "create":
		...
		strings.write_string(&body, `,"reviewer_agent_instance_id":"`); json_write_string(&body, option_value(args, "--reviewer-agent-instance-id", option_value(args, "--reviewer", ""))); strings.write_string(&body, `"`)
	...
	case:
		fmt.println("usage: ham-ctl tasks <create|list|next|show|log|comment|status|assign|participant|review>"); return
	}
...
ctl_task_chains :: proc(daemon_url, action: string, args: []string) {
	token := option_value(args, "--token", "")
	if token == "" { fmt.println("usage: ham-ctl task-chains <create|status|complete|show|retry-archives> --token <token> ..."); return }
	...
	if action == "create" {
		...
		strings.write_string(&body, `,"status":"`); json_write_string(&body, option_value(args, "--status", "")); strings.write_string(&body, `"`)
		strings.write_string(&body, `,"coordinator_agent_instance_id":"`); json_write_string(&body, option_value(args, "--coordinator-agent-instance-id", option_value(args, "--coordinator", ""))); strings.write_string(&body, `"`)
		strings.write_string(&body, `,"default_reviewer_agent_instance_id":"`); json_write_string(&body, option_value(args, "--default-reviewer-agent-instance-id", option_value(args, "--default-reviewer", ""))); strings.write_string(&body, `"`)
		...
		fmt.println("usage: ham-ctl task-chains <create|status|complete|show|retry-archives>"); return
	}
	strings.write_string(&body, `}`)
	response, ok := http.post(daemon_url, path, strings.to_string(body))
...
print_usage :: proc(config_path, daemon_url: string) {
	...
	fmt.println("  tasks create --token <token> --title <title> [--description <text>] [--assignee <agent>] [--reviewer <agent>] [--coordinator <agent>] [--status ready] [--chain-id <id>|--standalone]")
	fmt.println("    task_id is always generated by the daemon; root tasks auto-create a chain unless --standalone is set")
	fmt.println("    ready/status gating returns error=dependency|assignee_active_task with blocking_task_ids; wait for validation before next task when reviewer/verifier exists")
