...
update_conversation_title :: proc(s:^Content_Service, auth:contracts.Auth_Context,id,title:string)->(domain.Chat_Conversation,bool,domain.Domain_Error){ c,ok,err:=get_conversation(s,auth,id); if !ok do return {},false,err; next_title:=strings.trim_space(title); if next_title=="" do return {},false,domain.domain_error(.Validation_Failed,"conversation title is required"); if len(next_title)>120 do return {},false,domain.domain_error(.Validation_Failed,"conversation title is too long"); c.title=next_title; c.updated_at=platform.clock_now(s.clock); return iface.content_save_conversation(s.content,c) }
...
list_user_visible_messages :: proc(s:^Content_Service, auth:contracts.Auth_Context,cid:string,limit:int,cursor:string)->([]domain.Chat_Message,domain.Domain_Error){ c,ok,err:=get_conversation(s,auth,cid); if !ok do return nil,err; return iface.content_list_user_visible_messages(s.content,c.conversation_id,c.owner_user_id,limit,cursor) }
...
conversation_title_from_input :: proc(explicit_title, initial_body, fallback: string) -> string {
	seed := strings.trim_space(explicit_title)
	if seed == "" do seed = strings.trim_space(initial_body)
...
content_json_write :: proc(b:^strings.Builder, v:string){ for ch in v { switch ch { case '\\': strings.write_string(b,"\\\\"); case '"': strings.write_string(b,"\\\""); case '\n': strings.write_string(b,"\\n"); case '\r': strings.write_string(b,"\\r"); case '\t': strings.write_string(b,"\\t"); case: strings.write_rune(b,ch) } } }
