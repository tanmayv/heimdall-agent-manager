...
INSERT INTO chat_conversations_fts(rowid, title)
  SELECT rowid, title FROM chat_conversations
  WHERE rowid NOT IN (SELECT rowid FROM chat_conversations_fts);
...
INSERT INTO agents_fts(rowid, name, slug, instructions)
  SELECT rowid, name, slug, instructions FROM agents
  WHERE rowid NOT IN (SELECT rowid FROM agents_fts);
...
INSERT INTO agent_instances_fts(rowid, display_name, agent_id)
  SELECT rowid, display_name, agent_id FROM agent_instances
  WHERE rowid NOT IN (SELECT rowid FROM agent_instances_fts);
...
INSERT INTO task_chains_fts(rowid, title, description)
  SELECT rowid, title, description FROM task_chains
  WHERE rowid NOT IN (SELECT rowid FROM task_chains_fts);
...
INSERT INTO projects_fts(rowid, name, slug, description)
  SELECT rowid, name, slug, description FROM projects
...
INSERT INTO artifacts_fts(rowid, name, description)
  SELECT rowid, name, description FROM artifacts
  WHERE rowid NOT IN (SELECT rowid FROM artifacts_fts);
