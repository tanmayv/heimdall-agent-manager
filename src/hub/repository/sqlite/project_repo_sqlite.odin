...
project_get_sqlite :: proc(ctx: rawptr, project_id: domain.Project_ID) -> (domain.Project, bool, domain.Domain_Error) {
	...
	q := "SELECT project_id, owner_user_id, name, slug, description, repo_url, vcs_kind, default_path, state, created_at, updated_at FROM projects WHERE project_id = ?;"
...
project_list_by_owner_sqlite :: proc(ctx: rawptr, owner_user_id: domain.User_ID, limit: int, cursor: string) -> ([]domain.Project, domain.Domain_Error) {
	...
	q := "SELECT project_id, owner_user_id, name, slug, description, repo_url, vcs_kind, default_path, state, created_at, updated_at FROM projects WHERE owner_user_id = ? ORDER BY created_at DESC, project_id DESC LIMIT ?;"
	if cursor != "" do q = "SELECT project_id, owner_user_id, name, slug, description, repo_url, vcs_kind, default_path, state, created_at, updated_at FROM projects WHERE owner_user_id = ? AND created_at < ? ORDER BY created_at DESC, project_id DESC LIMIT ?;"
...
project_save_sqlite :: proc(ctx: rawptr, project: domain.Project) -> (domain.Project, bool, domain.Domain_Error) {
	impl := (^Project_Repo_SQLite)(ctx); stmt: sqlite3_stmt = nil
	q := "INSERT INTO projects (project_id, owner_user_id, name, slug, description, repo_url, vcs_kind, default_path, state, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(project_id) DO UPDATE SET name=excluded.name, slug=excluded.slug, description=excluded.description, repo_url=excluded.repo_url, vcs_kind=excluded.vcs_kind, default_path=excluded.default_path, state=excluded.state, updated_at=excluded.updated_at;"
	...
	bind_text(stmt, 1, string(project.project_id)); bind_text(stmt, 2, string(project.owner_user_id)); bind_text(stmt, 3, project.name); bind_text(stmt, 4, project.slug); bind_text(stmt, 5, project.description); bind_text(stmt, 6, project.repo_url); bind_text(stmt, 7, project.vcs_kind); bind_text(stmt, 8, project.default_path); bind_text(stmt, 9, domain.project_state_string(project.state)); bind_text(stmt, 10, project.created_at); bind_text(stmt, 11, project.updated_at)
