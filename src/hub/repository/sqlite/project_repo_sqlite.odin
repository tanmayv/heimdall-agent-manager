...
project_get_sqlite :: proc(ctx: rawptr, project_id: domain.Project_ID) -> (domain.Project, bool, domain.Domain_Error) {
	...
	q := "SELECT project_id, owner_user_id, name, slug, description, repo_url, vcs_kind, default_path, state, project_type, workspace_name, relative_path, created_at, updated_at FROM projects WHERE project_id = ?;"
...
project_list_by_owner_sqlite :: proc(ctx: rawptr, owner_user_id: domain.User_ID, limit: int, cursor: string) -> ([]domain.Project, domain.Domain_Error) {
	...
	q := "SELECT project_id, owner_user_id, name, slug, description, repo_url, vcs_kind, default_path, state, project_type, workspace_name, relative_path, created_at, updated_at FROM projects WHERE owner_user_id = ? ORDER BY created_at DESC, project_id DESC LIMIT ?;"
	if cursor != "" do q = "SELECT project_id, owner_user_id, name, slug, description, repo_url, vcs_kind, default_path, state, project_type, workspace_name, relative_path, created_at, updated_at FROM projects WHERE owner_user_id = ? AND created_at < ? ORDER BY created_at DESC, project_id DESC LIMIT ?;"
	...
	if cursor != "" {
		...
		bind_text(stmt, 3, i32_to_string_sqlite(effective_limit))
	} else {
		bind_text(stmt, 2, i32_to_string_sqlite(effective_limit))
	}
...
project_save_sqlite :: proc(ctx: rawptr, project: domain.Project) -> (domain.Project, bool, domain.Domain_Error) {
	impl := (^Project_Repo_SQLite)(ctx); stmt: sqlite3_stmt = nil
	q := "INSERT INTO projects (project_id, owner_user_id, name, slug, description, repo_url, vcs_kind, default_path, state, project_type, workspace_name, relative_path, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(project_id) DO UPDATE SET name=excluded.name, slug=excluded.slug, description=excluded.description, repo_url=excluded.repo_url, vcs_kind=excluded.vcs_kind, default_path=excluded.default_path, state=excluded.state, project_type=excluded.project_type, workspace_name=excluded.workspace_name, relative_path=excluded.relative_path, updated_at=excluded.updated_at;"
	...
	bind_text(stmt, 1, string(project.project_id)); bind_text(stmt, 2, string(project.owner_user_id)); bind_text(stmt, 3, project.name); bind_text(stmt, 4, project.slug); bind_text(stmt, 5, project.description); bind_text(stmt, 6, project.repo_url); bind_text(stmt, 7, project.vcs_kind); bind_text(stmt, 8, project.default_path); bind_text(stmt, 9, domain.project_state_string(project.state))
	ptype := project.project_type; if ptype == "" do ptype = "local"
	bind_text(stmt, 10, ptype)
	bind_text(stmt, 11, project.workspace_name)
	bind_text(stmt, 12, project.relative_path)
	bind_text(stmt, 13, project.created_at)
	bind_text(stmt, 14, project.updated_at)
...
project_from_stmt :: proc(stmt: sqlite3_stmt) -> domain.Project {
	...
	return domain.Project{
		...
		state = project_state_from_string(column_text(stmt, 8)),
		project_type = ptype,
		workspace_name = column_text(stmt, 10),
		relative_path = column_text(stmt, 11),
		created_at = column_text(stmt, 12),
		updated_at = column_text(stmt, 13),
	}
}
project_state_from_string :: proc(state: string) -> domain.Project_State { if state == "archived" do return .Archived; return .Active }
